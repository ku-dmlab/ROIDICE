...
def solve_mdp(mdp: MDP):
    ...
    for _ in range(1_000_000):
        ...
        pi_new[np.arange(mdp.num_states), np.argmax(q, axis=1)] = 1.0
...
def generate_trajectory(
    seed: int, cmdp: CMDP, pi: np.ndarray, num_episodes: int = 10, max_timesteps: int = 50
):
    ...
    for t in range(max_timesteps):
        ...
        for i in range(num_episodes):
            if not done[i]:
                trajectory[i].append((i, t, state[i], action[i], reward[i], costs[:, i], state1[i]))
...
def solve_cmdp(cmdp: CMDP):
    ...
    p = cmdp.transition.reshape(cmdp.num_states * cmdp.num_actions, cmdp.num_states)  # |S||A| x |S|
