...
def generate_baseline_policy(cmdp: util.CMDP,
  ...
  while util.policy_evaluation(cmdp, pi_soft)[0][0] > v_final_target:
    ...
    logging.info('temp=%.6f, R=%.3f, C=%.3f / v_opt=%.3f, f_target=%.3f', temperature, r[0], c[0][0], v_opt, v_final_target)
...
def targetROI(cmdp: util.CMDP,
                         pi_b: np.ndarray,
                         alpha: float, target: float):
  """f-divergence regularized constrained RL.
